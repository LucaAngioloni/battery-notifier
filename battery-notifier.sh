...
then
    /usr/local/bin/terminal-notifier -title "Battery Status" -message "Battery sufficiently charged (80%)" -appIcon "$icon_path" -group "battery_pwr" -sound "Hero"
fi
...
then
    /usr/local/bin/terminal-notifier -title "Battery Status" -message "Battery sufficiently charged (90%)" -appIcon "$icon_path" -group "battery_pwr" -sound "Hero"
fi
...
then
    /usr/local/bin/terminal-notifier -title "Battery Status" -message "Battery fully charged (100%)" -appIcon "$icon_path" -group "battery_pwr" -sound "Hero"
fi
